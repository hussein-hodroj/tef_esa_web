...
export const registerCandidate = asyncHandler(async (req, res) => {
  try {
    ...
    const emailDateCheckSQL = "SELECT COUNT(*) AS count FROM registrations WHERE Email = ? AND BookDate = ?";
    
      cnx.query(emailDateCheckSQL, [req.body.Email, req.body.BookDate], (err, data) => {
        if (data[0].count > 0) {
          return res.status(400).json({ message: 'Email and Date combination already exists in the database' });
        }else{
         Promise.all([
